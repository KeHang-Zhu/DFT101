...
def compute_energy(alat, nk, ecut, displ):
    """
...
def lattice_scan():
    ...
    ecut = 30
    alat = 3.88
    dis_list = numpy.linspace(0, 0.1, 11)
    print(dis_list)
    energy_list = []
    for dis in dis_list:
        output = compute_energy(alat=alat, ecut=ecut, nk=nk, displ = dis)
        energy_list.append(output["energy"])
        print(output)
    print(dis_list)
    print(energy_list)
    plt.plot(dis_list, energy_list)
    plt.show()

...
def lattice_scan():
    ...
    for dela in [0.0005,0.0006,0.0007,0.0008,0.0009,0.001]:
#         start_time =time.time()
        alat_list.append(dela)
        calc = make_calculator(nk=nk, ecut=ecut)
        delta_V = 3*5.652**2*dela
#         nk_list.append(nk)
        energy_list = []
        for alat in [5.652-dela, 5.652, 5.652+dela]:
#             alat_list.append(alat/0.5292)
...
#             time_list.append(cop_time)
        ...
        B_list.append((energy_list[0]+energy_list[2]- 2* energy_list[1])*5.652**3/delta_V**2*eV_GA)
    print(B_list)
...
#             print(ecut_list[i])
    ...
    plt.plot(alat_list, B_list,'.-')
    plt.xlabel('lattice parameter difference (A)')
    plt.ylabel('Bulk modulus of Ge (GPa)')
    plt.show()

...
def lattice_scan():
    ...
    for ecut in np.linspace(5, 80, 16):
        calc = make_calculator(nk=nk, ecut=ecut)
        ecut_list.append(ecut)
        energy_list = []
        for alat in [5.662, 5.689]:
            atoms = make_struc(alat=alat)
            atoms[0].position[2] += 0.05
#             print(atoms[0].position[2], atoms[1].position[2])
...
#             forces = atoms.get_forces()
        ...
        e_diff_list.append(np.abs(energy_list[0] - energy_list[1]))
#         energy_list=[]
        
            
    print(e_diff_list)
    print(ecut_list)
    for i in range(len(ecut_list)-1):
        if np.abs(e_diff_list[i+1] - e_diff_list[i]) < 5e-3 *units.Ry * 2:
            print(ecut_list[i])
    plt.plot(ecut_list, e_diff_list,'.-')
    plt.xlabel('Cutoff energy (Ry)')
    plt.ylabel('Energy difference (Ry)')
